...
class SimulationService:
    ...
    def conduct_simulation(self, results_file_suffix="results.json"):
        ...
        Allows the user to select which question file to use for the simulation.
        Results are appended to the results_file.

        Args:
            results_file_suffix (str): The suffix for the results file (e.g., "results.json").
        """
        # List available question files from the 'files' directory
        ...
        for i, file_path in enumerate(question_files):
            print(f"{i + 1}. {file_path.name}") # Display only the file name
        ...
        questions_source_file = str(selected_file_path) # selected_file_path already includes 'files/'
        
        selected_exam_code = selected_file_path.stem.replace("questions_", "") 
        # Construct results file path within the 'files' directory
        ...
        try:
            ...
            print(f"Error: Questions file '{questions_source_file}' not found. This should not happen after selection.")
            return
        ...
        print(f"\n--- Starting Simulation for Exam: {exam_code} (Source: {questions_source_file}) ---")
        ...
        all_results = []
        if results_file.exists(): # Use Path object's exists()
            try:
